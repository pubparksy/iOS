...
app.post("/habit_add", (req, res)=>{
    if (req.session.user) {
        ...
        db.run(sql, (err)=>{
            if (err) {
                res.status(500).send("Interval Server Error - habit insert");
            } else {
...
app.post("/habit_record_add", (req, res)=>{
    if (req.session.user) {
        const { memo } = req.body;
        const { id, name } = req.session.user;
        const createdAt = moment().format("YYYY-MM-DD");
        ...
        db.run(sql, (err)=>{
            if (err) {
                ...
                res.redirect(`/record/${id}`); // 처리 끝내고 화면전환 (render는 기존 list db.all하고 다시 담아야해서 안 쓰고 하면 오류남?)
            }
        })
    } else {
        res.redirect("/login");
